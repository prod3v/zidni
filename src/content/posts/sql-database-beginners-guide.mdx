...
authors: ["فريق زدني"]
tags: ["قواعد بيانات", "SQL", "برمجة", "تعلم برمجة"]
draft: false
---
...
لاحظ أننا لم نحدد قيمة لـ `student_id` لأنه `AUTO_INCREMENT` — قاعدة البيانات تتكفّل بتوليد الرقم تلقائيًا. وكذلك لم نحدد `enrollment_date` لأننا وضعنا له قيمة افتراضية.
...
- **المفتاح الأساسي (Primary Key):** عمود (أو مجموعة أعمدة) يُعرّف كل صف بشكل فريد. مثل الرقم الجامعي — لا يتكرر
...
- لا يمكنك تسجيل طالب غير موجود أو في مادة غير موجودة — قاعدة البيانات ترفض العملية
