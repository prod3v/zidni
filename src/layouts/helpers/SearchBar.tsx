  ...
  return (
    <div className="min-h-[45vh]">
      <input
        ...
        placeholder="اكتب هنا للبحث في المقالات"
        type="text"
        name="search"
        value={inputVal}
      ...
      {inputVal.length > 1 && (
        <div className="my-6 text-center">
          تم العثور على {searchResults?.length}
        ...
        {searchResults?.map(({ item }) => (
          <div key={item.slug} className={"col-12 mb-8 sm:col-6"}>
            ...
            <ul className="mt-6 mb-4 flex flex-wrap items-center text-text">
              <li className="me-5 flex items-center flex-wrap font-medium">
                <BiCalendarEdit className="me-1 h-5 w-5 text-gray-600" />
                <>{dateFormat(item.data.date)}</>
              </li>
              <li className="me-5 flex items-center flex-wrap">
                <BiCategoryAlt className="me-1 h-[18px] w-[18px] text-gray-600" />
                <>
                  <ul>
                    {item.data.categories.map((category: string, i: number) => (
                      <li key={i} className="inline-block">
                        <a
                          ...
                          className="me-2 hover:text-primary font-medium"
                        >
                          {humanize(category)}
